"""
Test residual connections using DCAdd modules.

This demonstrates the proper way to implement residual connections
for DC decomposition using DCAdd modules instead of + operator.
"""
...
class DCSimpleResidual(nn.Module):
    """Residual connection using DCAdd module."""
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(16, 16, 3, padding=1)
        self.conv2 = nn.Conv2d(16, 16, 3, padding=1)
        self.add = DCAdd()  # DC-compatible addition
    ...
    def forward(self, x):
        ...
        out = torch.relu(out)
        out = self.conv2(out)
        out = self.add(out, identity)  # Use DCAdd instead of +
        return torch.relu(out)


class DCLinearResidual(nn.Module):
    """Linear residual connection using DCAdd."""
    def __init__(self):
        super().__init__()
...
class DCMultipleAdditions(nn.Module):
    """Multiple tensor additions using DCAdd."""
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(8, 8, 3, padding=1)
        self.conv2 = nn.Conv2d(8, 8, 3, padding=1)
        self.conv3 = nn.Conv2d(8, 8, 3, padding=1)
        self.add1 = DCAdd()  # First addition
        self.add2 = DCAdd()  # Second addition
    ...
    def forward(self, x):
        ...
        # Multiple additions using DCAdd modules
        out = self.add1(branch1, branch2)
        out = self.add2(out, branch3)
        return torch.relu(out)


class DCNestedResidual(nn.Module):
    """Nested residual connections using DCAdd."""
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        # First residual
        out1 = self.conv1(x)
        out1 = self.add1(out1, x)
        out1 = torch.relu(out1)
        
        # Second residual
        out2 = self.conv2(out1)
        out2 = self.add2(out2, out1)
        out2 = torch.relu(out2)
        
        # Third residual
        out3 = self.conv3(out2)
        out3 = self.add3(out3, out2)
        out3 = torch.relu(out3)
        
        return out3


def get_dc_residual_models():
    """Get DC residual connection test models."""
    models = {}
    
    models['DCSimpleResidual'] = (DCSimpleResidual(), torch.randn(1, 16, 8, 8))
    models['DCLinearResidual'] = (DCLinearResidual(), torch.randn(2, 64))
    models['DCMultipleAdditions'] = (DCMultipleAdditions(), torch.randn(1, 8, 16, 16))
    models['DCNestedResidual'] = (DCNestedResidual(), torch.randn(1, 32, 8, 8))
    
    return models


def run_dc_residual_tests():
    """Run DC residual connection tests."""
    print("🧪 DC Decomposition - DCAdd Residual Connection Tests")
    print("=" * 60)
    
    models = get_dc_residual_models()
    results = {}
    ...
    print("=" * 60)
    print(f"DC RESIDUAL CONNECTION SUMMARY: {successful}/{total} tests passed ({100*successful/total:.1f}%)")
    
    if successful == total:
        print("🎉 All DC residual connection tests passed!")
